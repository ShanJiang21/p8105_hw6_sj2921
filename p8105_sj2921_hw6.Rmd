...
#### based on a hypothesized structure for the factors that underly birthweight
```{r}
...
#### Combination of birthweight literature and data-driven model-building 

```{r}

```

(2) Stepwise elimination 

Start with a full regression model for birthweight.

```{r}
## Stepwise with full model, after excluding two variables 
full_mlr = birth_weight %>% 
  lm(bwt ~  babysex + frace + mrace + malform +
     bhead + blength + fincome + gaweeks+ menarche + mheight + momage + 
     parity + ppbmi + smoken + wtgain, data = . )
...
* The full model has an R-square of 0.7116 and the adjusted R-square is 0.7106. 
